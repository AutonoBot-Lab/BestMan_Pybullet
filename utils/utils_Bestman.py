...
class Bestman:
    ...
    # option 1: Manhattan paths
    # option 2: Navigation paths
        ...
        for x in range(n_points_x):
            for y in range(n_points_y):
                ...
                for dx, dy in DIRECTIONS_8:
                    nx, ny = x + dx, y + dy
                    if 0 <= nx < n_points_x and 0 <= ny < n_points_y:
                        # 对于对角方向，确保两个相关的直线方向都没有障碍物
                        if (dx, dy) in [(1, 1), (1, -1), (-1, -1), (-1, 1)]:
                            if static_map[x + dx, y] == 1 or static_map[x, y + dy] == 1:
                                continue
                        if static_map[nx, ny] != 1:
                            graph.add_edge((x, y), (nx, ny), weight=COSTS[(dx, dy)])

        path = nwx.astar_path(
            graph, 
            tuple(init_base_position_grid), 
            tuple(goal_base_position_grid), 
            heuristic=lambda u, v: math.sqrt((u[0] - v[0]) ** 2 + (u[1] - v[1]) ** 2)
        )
