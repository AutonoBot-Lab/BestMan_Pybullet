...
class navigation:
    def __init__(self, bestman):
        self.bestman = bestman
    ...
    def A_star(
        ...
        init_base_position = self.bestman.get_base_pose().position[0:2]   # only care about x, y
        # init_base_position = init_base_position.position[0:2]   # only care about x, y
        goal_base_position = goal_base_pose.position[0:2]
        ...
        (
            ...
            max_y_base,
            _,
        ) = self.bestman.pb_client.get_bounding_box(self.bestman.base_id)
        ...
        (
            ...
            max_y_arm,
            _,
        ) = self.bestman.pb_client.get_bounding_box(self.bestman.arm_id)
        ...
        if not enable_accurate_occupancy_map:
            # get occupancy map
            for obstacle_id in self.bestman.pb_client.obstacle_navigation_ids:
                aabb = self.bestman.pb_client.get_bounding_box(obstacle_id)
                for i in range(
            ...
            for obstacle_id in self.bestman.pb_client.obstacle_navigation_ids:
                link_ids = [
                    i
                    for i in range(
                        -1, p.getNumJoints(obstacle_id, physicsClientId=self.bestman.client_id)
                    )
                ]
                for link_id in link_ids:
                    aabb_link = p.getAABB(
                        obstacle_id, link_id, physicsClientId=self.bestman.client_id
                    )
    ...
    # TODO: Add other navigation algorithms