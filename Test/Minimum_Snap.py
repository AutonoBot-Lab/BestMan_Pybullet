...
def minimum_snap_trajectory_2d(waypoints, times):
    ...
    objective = cp.Minimize(
        sum(
            cp.sum_squares(24 * coeffs[i][4] + 120 * coeffs[i][5] * (times[i+1] - times[i]) +
    ...
    for i in range(n - 1):
        for dim in range(d):
            constraints += [
                coeffs[i][0, dim] + coeffs[i][1, dim] * (times[i+1] - times[i]) +
                ...
                == coeffs[i+1][0, dim],
                coeffs[i][1, dim] + 2 * coeffs[i][2, dim] * (times[i+1] - times[i]) +
                ...
                == coeffs[i+1][1, dim],
                2 * coeffs[i][2, dim] + 6 * coeffs[i][3, dim] * (times[i+1] - times[i]) +
                ...
                == 2 * coeffs[i+1][2, dim],
                6 * coeffs[i][3, dim] + 24 * coeffs[i][4, dim] * (times[i+1] - times[i]) +
    ...
    for i in range(1, n):
        for dim in range(d):
            constraints.append(
                coeffs[i][0, dim] == waypoints[i, dim]
            )
    ...
    if prob.status not in ["infeasible", "unbounded"]:
        ...
        for i in range(n):
            ...
            segment = np.array([np.poly1d(coeffs[i].value[:, dim][::-1])(t - times[i]) for dim in range(d)]).T
            traj.append(segment)
